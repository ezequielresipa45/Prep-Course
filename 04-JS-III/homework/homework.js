...
function incrementarPorUno(array) {
  ...
  for (let index = 0; index < array.length; index++) {
    arregloNuevo.push(array[index] + 1);
  }
...
function cuentoElementos(arreglo) {
  ...
  for (let index = 0; index < arreglo.length; index++) {
    if (arreglo[index] > 18) {
      contadorElementos.push(arreglo[index]);
    }
  }

  return contadorElementos.length;
}
...
function mesesDelAño(array) {
  ...
  if (
    ...
    nuevoArregloFiltrado = array.filter(
      (element) =>
        element === "Marzo" || element === "Enero" || element === "Noviembre"
    );

    return nuevoArregloFiltrado;
  } else {
